...
__title__ = 'Number Balconies\nwith Paths (Numbers)'
__doc__ = "Assigns Balcony numbers automatically using the 'Label Maker' line style as the path of order. Deletes 'Label Maker' lines at completion."
...
class IForm(Form):
    ...
    def __init__(self):
        ...
        head = Label(Text = 'Options for automatically numbering Balconies with \'Item No\' BAL, using the "Label Maker" Line style. Lines will be deleted after completion')
...
for el in balcony:
    ...
    balconyBounds.append(bounds)

orderedElements = []

for xy in ptList:
    for alpha in range(len(balcony)):
        if balcony[alpha] not in orderedElements:
            if (xy.X > balconyBounds[alpha][0].X and xy.X < balconyBounds[alpha][1].X):
                if (xy.Y > balconyBounds[alpha][0].Y and xy.Y < balconyBounds[alpha][1].Y):
                    orderedElements.append(balcony[alpha])
                    break
...
with DB.Transaction(doc, "Numbering Banlconies") as trans:
