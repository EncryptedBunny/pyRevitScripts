...
__title__ = 'Number Doors\nwith Paths'
__doc__ = "Assigns Doors numbers automatically using the 'Label Maker' line style as the path of order. Deletes 'Label Maker' lines at completion."
...
class IForm(Form):
    ...
    def __init__(self):
        ...
        head = Label(Text = 'Options for automatically numbering Doors bays using the "Label Maker" Line style. Lines will be deleted after completion')
...
parking = DB.FilteredElementCollector(doc,doc.ActiveView.Id).OfCategory(DB.BuiltInCategory.OST_Doors).WhereElementIsNotElementType().ToElements()
parkingBounds = []
...
with DB.Transaction(doc, "Numbering Doors") as trans:
