...
__title__ = 'Number Rooms\nwith Paths'
__doc__ = "Assigns Room numbers automatically using the 'Label Maker' line style as the path of order. Deletes 'Label Maker' lines at completion."
...
class IForm(Form):
    ...
    def __init__(self):
        ...
        head = Label(Text = 'Options for automatically numbering Room Numbers using the "Label Maker" Line style. Lines will be deleted after completion')
...
rooms = []
for a in DB.FilteredElementCollector(doc,doc.ActiveView.Id).OfCategory(DB.BuiltInCategory.OST_Rooms).WhereElementIsNotElementType().ToElements():
...
for el in rooms:
    ...
    roomBounds.append(bounds)

orderedElements = []

for xy in ptList:
    for alpha in range(len(rooms)):
        if rooms[alpha] not in orderedElements:
            if (xy.X > roomBounds[alpha][0].X and xy.X < roomBounds[alpha][1].X):
                if (xy.Y > roomBounds[alpha][0].Y and xy.Y < roomBounds[alpha][1].Y):
                    orderedElements.append(rooms[alpha])
                    break
...
with DB.Transaction(doc, "Numbering Rooms") as trans:
    
    trans.Start()

    for thing in orderedElements:
        if counter < 10:
            thing.GetParameters("Room-Apartment Number")[0].Set(prefix + "0" + "%s" % counter + suffix)
            counter += 1
        else:
