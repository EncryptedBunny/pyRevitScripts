...
__doc__ = 'Moves the Room-Apartment number from the area into all Furniture families which are contained within that area.'
...
class IForm(Form):
    ...
    def __init__(self, dataEnteringNode):
        ...
        lb = Label(Text = 'Select Parameter to Assign to Furniture')
        ...
        def update():
            ...
            for i in range(self.prog.Maximum):
                def step():
                    ...
                    try:
                        collector = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Furniture).WhereElementIsNotElementType()
                        furniture = (collector.WherePasses(DB.ElementIntersectsSolidFilter(areasGeo[i])).ToElements())
...
elementsToCheck = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Furniture).WhereElementIsNotElementType().ToElements()
...
#Transaction
with DB.Transaction(doc, 'Area to Furniture') as trans:
    trans.Start()
    
    # Start Progress Bar
    Application.EnableVisualStyles()
    f = ProgressBarDialog(len(areasGeo), 'Transfering Parameters to Furniture')
    f.ShowDialog()
