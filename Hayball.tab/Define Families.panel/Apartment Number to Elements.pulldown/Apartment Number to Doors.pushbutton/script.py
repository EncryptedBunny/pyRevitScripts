...
__doc__ = 'Moves the Room-Apartment number from the area into all Door families which are contained within that area.'
...
# Inputs
inputList = ['Room-Apartment Number']
...
class IForm(Form):
    ...
    def __init__(self, dataEnteringNode):
        ...
        lb = Label(Text = 'Select Parameter to Assign to Doors')
            ...
            for i in range(self.prog.Maximum):
                def step():
                    ...
                    try:
                        ...
                        doors = (collector.WherePasses(DB.ElementIntersectsSolidFilter(areasGeo[i])).ToElements())
                        for item in doors:
...
elementsToCheck = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Doors).WhereElementIsNotElementType().ToElements()
...
#Transaction
with DB.Transaction(doc, 'Area to Doors') as trans:
    trans.Start()
    
    # Start Progress Bar
    Application.EnableVisualStyles()
    f = ProgressBarDialog(len(areasGeo), 'Transfering Parameters to Doors')
    f.ShowDialog()
