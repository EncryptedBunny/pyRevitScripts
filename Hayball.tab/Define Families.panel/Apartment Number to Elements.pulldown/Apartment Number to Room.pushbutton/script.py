...
__doc__ = 'Moves the Room-Apartment number from the area into all Rooms families which are contained within that area.'
...
class IForm(Form):
    ...
    def __init__(self, dataEnteringNode):
        ...
        lb = Label(Text = 'Select Parameter to Assign to Rooms')
        ...
        def update():
            ...
            for i in range(self.prog.Maximum):
                def step():
                    ...
                    for item in Rooms:
                        for obj in item.ClosedShell:
                            room = obj
                        
                        try:
                            intersect = DB.BooleanOperationsUtils.ExecuteBooleanOperation(room, areasGeo[i], DB.BooleanOperationsType.Intersect)
                                                
                            if intersect.Volume > 0.000001:
                                try:
                                    item.GetParameters(parameterName)[0].Set((str)(areasName[validIndex[i]]))
                                except:
                                    None
                        except:
                            None
...
elementsToCheck = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Rooms).WhereElementIsNotElementType().ToElements()
...
#Transaction
with DB.Transaction(doc, 'Area to Rooms') as trans:
    trans.Start()
    
    # Start Progress Bar
    Application.EnableVisualStyles()
    f = ProgressBarDialog(len(areasGeo), 'Transfering Parameters to Rooms')
    f.ShowDialog()
