...
__doc__ = 'Moves the Room-Apartment number from the area into all Window families which are contained within that room.'
...
for i in DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Windows).WhereElementIsNotElementType().ToElements():
...
#Transaction
with DB.Transaction(doc, 'Room to Window') as trans:
    trans.Start()
    
    # Start Progress Bar
    Application.EnableVisualStyles()
    f = ProgressBarDialog(len(elementsToCheck), 'Transfering Parameters to Window')
    f.ShowDialog()
