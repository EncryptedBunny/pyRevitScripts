...
__doc__ = 'Moves the Room-Apartment number from the area into all Casework families which are contained within that area.'
...
class IForm(Form):
    ...
    def __init__(self, dataEnteringNode):
        ...
        lb = Label(Text = 'Select Parameter to Assign to Casework')
        ...
        def update():
            ...
            for i in range(self.prog.Maximum):
                def step():
                    ...
                    try:
                        collector = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Casework).WhereElementIsNotElementType()
                        casework = (collector.WherePasses(DB.ElementIntersectsSolidFilter(areasGeo[i])).ToElements())
                        for item in casework:
...
elementsToCheck = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Casework).WhereElementIsNotElementType().ToElements()
...
#Transaction
with DB.Transaction(doc, 'Area to Casework') as trans:
    trans.Start()
    
    # Start Progress Bar
    Application.EnableVisualStyles()
    f = ProgressBarDialog(len(areasGeo), 'Transfering Parameters to Casework')
    f.ShowDialog()
